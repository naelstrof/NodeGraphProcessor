...
internal class CreateNodeMenuWindow : ScriptableObject, ISearchWindowProvider {
    ...
    public bool OnSelectEntry(SearchTreeEntry searchTreeEntry, SearchWindowContext context) {
        ...
        var windowMousePosition =
            windowRoot.ChangeCoordinatesTo(windowRoot.parent, context.screenMousePosition - window.position.position);
    ...
    private void CreateEdgeNodeMenu(List<SearchTreeEntry> tree) {
        ...
        var sortedMenuItems = entries
            .Select(port => (port, nodePaths.FirstOrDefault(kp => kp.type == port.nodeType).path)).OrderBy(e => e.path);
