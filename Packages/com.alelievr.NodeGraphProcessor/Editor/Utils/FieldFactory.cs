...
public static class FieldFactory {
    ...
    static FieldFactory() {
        foreach (var type in AppDomain.CurrentDomain.GetAllTypes()) {
            var drawerAttribute =
                type.GetCustomAttributes(typeof(FieldDrawerAttribute), false).FirstOrDefault() as FieldDrawerAttribute;
    ...
    private static void AddDrawer(Type fieldType, Type drawerType) {
        ...
        if (!iNotifyType.IsAssignableFrom(drawerType)) {
            Debug.LogWarning("The custom field drawer " + drawerType + " does not implements INotifyValueChanged< " +
                             fieldType + " >");
            return;
        }
    ...
    public static VisualElement CreateField(Type fieldType, object value, Action<object> onValueChanged, string label) {
        ...
        if (fieldType == typeof(LayerMask)) {
            ...
            try {
                ...
                try {
                    field =
                        createFieldSpecificMethod.Invoke(null, new[] { value, onValueChanged, label }) as VisualElement;
                } catch {
                }
